...
def leg_points(side, pair, activation, actions):
    ...
    p1 = anchor + np.array([0.48 * side_sign, 0.34 * y_dir, -0.10 + 0.16 * vertical])

    # Deliberately exaggerated for visualization.  This does not feed back
    # into the neural simulation or alter spike data.
...
def main():
    ...
    parser.add_argument("--inactive-alpha", type=float, default=0.02)
    parser.add_argument("--active-alpha", type=float, default=0.95)
    parser.add_argument(
        "--motion-gain", type=float, default=6.0,
        help="Visual-only amplification of motor-driven fly motion",
    )
    parser.add_argument(
        "--max-skeletons", type=int, default=20,
        help="Maximum cached neuron morphologies to load/render",
    )
    parser.add_argument(
        "--show-inactive", action="store_true",
        help="Render inactive neuron skeletons faintly (slower)",
    )
    ...
    if args.max_skeletons > 0:
        selected = selected.head(args.max_skeletons).copy()
    ...
    selected_ids = set(skeletons.keys())
    ...
    print(
        f"      render mode: {'all skeletons' if args.show_inactive else 'active skeletons only'}",
        flush=True,
    )
    print(f"      visual motion gain: {args.motion_gain:g}x", flush=True)
    ...
    # ---- fly panel ----------------------------------------------------
    ax_fly.set_title(f"Procedural fly / motor output (visual gain ×{args.motion_gain:g})")
    ...
    def update_fly(frame):
        ...
        wing_level = float(np.clip(
            motor_activation[frame, groups.index("wing")] * args.motion_gain,
            0.0, 1.0,
        ))
        ...
        for i, ((side, pair), line) in enumerate(zip(leg_order, leg_lines)):
            visual_activation = np.clip(
                leg_activation[frame, i] * args.motion_gain,
                0.0, 1.0,
            )
            pts = leg_points(side, pair, visual_activation, actions)
